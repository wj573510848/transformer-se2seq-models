...
class Data:
    ...
    def pre_process_data(raw_data,tokenizer,config,logger):
        ...
        if os.path.isfile(source_vocab_file) and  os.path.isfile(target_vocab_file) and os.path.isfile(sample_file):
            ...
            logger.info("source vocabulary size:{}".format(len(source_vocab)+len(special_vocab)+len(config.vocab_remains)))
            logger.info("target vocabulary size:{}".format(len(target_vocab)+len(special_vocab)+len(config.vocab_remains)))
